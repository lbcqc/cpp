...
int main(int argc, char **argv) {
  // std::cout << foo("abc","abc") << std::endl; // error: invalid operands of types 'const char*' and 'const char*' to
  // binary 'operator*'
  std::cout << foo(2, 3) << std::endl;
  // std::cout << foo_integral(2.0,3.0) << std::endl; // note: the expression 'is_integral_v<T> [with T = double]'
  // evaluated to 'false'
