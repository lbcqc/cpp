...
// 解法：只需要遇到(时，将-1 +1入栈，遇到)时将栈顶的+1
// -1出栈，然后每次遇到数字，就取栈顶符号*当前符号*当前数字做累加，得到最终的结果
// 特别注意：虽然题目保证每个数字和结果都在 int32 范围内，但是表达式的中间结果，可能会超 int32，故而需要 Long
// 来保存中间结果 特别注意：sign是当前符号，需要作用域符号*当前符号，即遇到+则为栈顶，遇到-则为-栈顶。
